    ...
    async function carregarFlags() {
      try {
        const resposta = await fetch('../assets/flags-div.json'); 
        if (!resposta.ok) throw new Error('Erro ao carregar JSON: ' + resposta.status);
        const FLAGS = await resposta.json();        
        console.log('Flags carregadas:', FLAGS);

        iniciarCTF(FLAGS);
      } catch (erro) {
        console.error('Não foi possível carregar as flags', erro);
      }
    }
